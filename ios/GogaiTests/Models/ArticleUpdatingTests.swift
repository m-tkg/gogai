...
final class ArticleUpdatingTests: XCTestCase {
    private func makeArticle(isRead: Int = 0, isFavorite: Int = 0, readAt: String? = nil) -> Article {
        Article(id: 1, feed_id: 2, guid: "guid", title: "Title",
                link: "https://example.com", summary: "summary", content: "content",
                published_at: "2026-01-01T00:00:00Z", is_read: isRead, is_favorite: isFavorite,
                created_at: "2026-01-01T00:00:00Z", read_at: readAt)
    }

    func test_引数なしは同じ内容を返す() {
        let article = makeArticle(isRead: 1, isFavorite: 1, readAt: "2026-01-02T00:00:00Z")
        XCTAssertEqual(article.updating(), article)
    }

    func test_isReadのみ変更し他のフィールドは保持する() {
        let article = makeArticle(isFavorite: 1, readAt: "2026-01-02T00:00:00Z")
        let updated = article.updating(isRead: 1)
        XCTAssertEqual(updated.is_read, 1)
        XCTAssertEqual(updated.is_favorite, 1)
    ...
    func test_isFavoriteのみ変更できる() {
        let article = makeArticle(isRead: 1)
        let updated = article.updating(isFavorite: 1)
        XCTAssertEqual(updated.is_favorite, 1)
        XCTAssertEqual(updated.is_read, 1)
    }
